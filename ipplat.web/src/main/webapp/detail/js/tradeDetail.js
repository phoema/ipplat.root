	...
	$.ajax({
		...
		success:function(data){    
			...
			if(objor=='null'|| objor==null){ 
				...
				$('#tip0').html(obj.mno);
				$(".location span[name='title']").text(obj.mno);
				$('#tip1').html(obj.cs);  
				...
				$('#ncnum').attr('href','../list/trademarklist.html?ex='+encodeURIComponent(' (尼斯分类 =(\''+obj.nc+'\') ) ')+'');
				...
				$('#appealTrademark').attr('href','../list/trademarklist.html?ex='+encodeURIComponent(' (申请人名称= (\''+applicants.hno+' \') )')+'');
				
				$('#dailiTrademark').html(obj.aro);//代理人名称
				$('#dailiTrademark').attr('href','../list/trademarklist.html?ex='+encodeURIComponent(' (代理人名称= (\''+obj.aro+'\' ) )')+''); 
