$(document).ready(function() {
	...
	$(".showNumSelect_Div").click(function() {
		...
		$("#showNumSelect").css("background-image", "url('/" + rootPath + "module/di/img/patent/table/arrowBlueDown.png')");
		return false;
	});
	...
	$("#showNumSelect_ul li").click(function() {
		...
		$("#showNumSelect").css("background-image", "url('/" + rootPath + "module/di/img/patent/table/arrowBlueuP.png')");
	...
	$("#showHisLIst").click(function() {
		...
		if ($("#HisList").hasClass("displayNone")) {
			...
			$(".hisListImg").attr("src", "/" + rootPath + "module/di/img/patent/table/hisUp.png");
			mGrid_gridHis_obj.query(1, function() {
				hisRecordComplete();
			});
		} else {
			...
			$(".hisListImg").attr("src", "/" + rootPath + "module/di/img/patent/table/hisDown.png");
		}
	});
	...
	// copyToClipboard(($(".ExpressText").text() + ""));
	// });

	var clip = new ZeroClipboard(document.getElementById("hisCopyBtn"), {
		moviePath : '/'
	});
	...
	clip.addEventListener("mouseDown", function(client) {
		client.setText($(".ExpressText").text() + "");
	});
	...
	// if ($(this).is(':checked') == true) {
	// $("input[name='md5']").attr("checked", "true");
	// } else {
	// $("input[name='md5']").removeAttr("checked");
	// }
	// });
	$("#selectAllMD5").click(function() {
...
function createLibrarySearchTable(express) {
	...
	if (txnCode == "PatentQuoteSea") {
		regExp = /(PDB =)(.*?)(\')(.*?)(\')/gi;
		while (match = regExp.exec(express)) {
			result.push(match[4]);
		}
	}
	if (txnCode == "PatentExpandSea") {
		regExp = /(PDB =)(.*?)(\')(.*?)(\')/gi;
		while (match = regExp.exec(express)) {
			result.push(match[4]);
		}
	...
	if (result == null || result.length == 0 || result.length == 47) {
		if(freezeTxnAction == "WRegistrationTable"){
			$(".LibrarySearchTable").append("<tr><td colspan=\"5\" style=\"line-height: normal; padding:20px; font-weight: bold;\">当前检索选择了软著登记公告库。</td></tr>");
		}else if(freezeTxnAction == "TRegistrationTable"){
			$(".LibrarySearchTable").append("<tr><td colspan=\"5\" style=\"line-height: normal; padding:20px; font-weight: bold;\">当前检索选择了软著转让登记公告库。</td></tr>");
		}else if(freezeTxnAction == "CRegistrationTable"){
			$(".LibrarySearchTable").append("<tr><td colspan=\"5\" style=\"line-height: normal; padding:20px; font-weight: bold;\">当前检索选择了软著登记撤销库。</td></tr>");
		}else if(freezeTxnAction == "ARegistrationTable"){
			$(".LibrarySearchTable").append("<tr><td colspan=\"5\" style=\"line-height: normal; padding:20px; font-weight: bold;\">当前检索选择了软著专用许可合同登记公告库。</td></tr>");
		}else if(freezeTxnAction == "URegistrationTable"){
			$(".LibrarySearchTable").append("<tr><td colspan=\"5\" style=\"line-height: normal; padding:20px; font-weight: bold;\">当前检索选择了软著变更或补充公告库。</td></tr>");
		}else if(freezeTxnAction == "WProductionTable"){
			$(".LibrarySearchTable").append("<tr><td colspan=\"5\" style=\"line-height: normal; padding:20px; font-weight: bold;\">当前检索选择了作品著作权库。</td></tr>");
		}else{
			$(".LibrarySearchTable").append("<tr><td colspan=\"5\" style=\"line-height: normal; padding:20px; font-weight: bold;\">当前检索选择了所有检索库，包括（中国专利、美国专利、日本专利、韩国专利、德国专利、澳大利亚专利、EPO专利、加拿大专利、俄罗斯专利、英国专利、瑞士专利、法国专利、WIPO专利、其它国家和地区专利）</td></tr>");
		}
	...
	for ( var i = 0; i < LibrarySearchNodesLength; i++) {
		...
		for ( var j = 0; j < 5; j++) {
			...
			if (txnCode == "PatentTableSea") {
				...
				if(library == "其他国家和地区专利"){
					library = library.substring(0,library.length - 2);
				}
			}
			if (txnCode == "PatentQuoteSea") {
				library = patentLibrary[res];
...
function addExpressTip(obj) {
	...
	if ($(obj).hasClass("seaHistoryOperate0")) {
		var v = $(obj).attr("v") + "";
		var oldExpress = $(".editFieldList").val();
		if($.trim(oldExpress) != ""){
			oldExpress = oldExpress + " AND " + $("#" + v).text();
		}else{
			oldExpress = $("#" + v).text();
		}
		$(".editFieldList").val(oldExpress);
		$("html,body").animate({
