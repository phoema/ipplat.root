			...
			if ($.trim(strs[i]) != "") {
				if (i != 0) {
					link += "<a style='margin-left:5px;' href=\""
							...
							+ encodeURIComponent("( " + str + " = '"
									+ strs[i].replace(/\'/g, "\\'") + "' )")
							+ " \" target=\"_blank\" >" + strs[i] + " "
							+ txtDate.substring(0, 4) + "."
							+ txtDate.substring(4, 6) + "."
							+ txtDate.substring(6, 8) + "</a>";
				} else {
					link += "<a href=\""
							+ action
							+ ".do?select-key%3AexpressCN="
...
function buildLawLink(obj, str) {
	...
	if (txt != null && txt != ("")) {
		...
		if ($.trim(txt) != "") {
			link += "<a href=\"txnLawsList.do?select-key%3AexpressCN="
					+ encodeURIComponent("( " + str + " = '"
							+ linkTxt.replace(/\'/g, "\\'") + "' )")
					+ " \" target=\"_blank\" >" + txt + "</a>&nbsp;&nbsp;";
		}
...
function buildLinkRelation(obj, str, action) {
	...
	if (txt != null && txt != ("")) {
		...
		if ($.trim(txt) != "" && txt != "0") {
			link = "<a href=\"" + action + ".do?select-key%3AexpressCN="
					+ encodeURIComponent("( " + str + " )")
					+ " \" target=\"_blank\" title=\"检索 " + "\" >" + txt
					+ "</a>&nbsp;&nbsp;";
			$(obj).append(link);
		} else {
			$(obj).append("0");
	...
	if (eprys1 != null && eprys1 != "") {
		...
		for (var i = 0; i < eprys.length; i++) {
			if (eprys[i] != null && eprys[i] != " " && eprys[i] != "") {
				...
				dataType = dataType.substr(0, 4) + "." + dataType.substr(4, 2)
						+ "." + dataType.substr(6, 2);
				eprys[i] = eprys[i].substr(0, eprys[i].length - 8) + " "
						+ dataType;
				link = "<a href='' class='" + str
						+ "Click cursorPointer' style='margin-right:5px;'>"
						+ eprys[i] + "</a>";
				$(obj).append(link);
			}
		...
		if ($.trim(txt) != "") {
			link = "<a href=\""
					...
					+ encodeURIComponent("( " + str1 + " = '"
							+ txtneirong.replace(/\'/g, "\\'") + "' and "
							+ str2 + " = '"
							+ txtneirongapy.replace(/\'/g, "\\'") + "' and "
							+ str3 + " = '"
							+ txtneirongiss.replace(/\'/g, "\\'") + "')")
					+ " \" target=\"_blank\" title=\"检索 " + txtneirong + "  "
					+ txtneirongapy + "  " + txtneirongiss + "期" + "\" >" + txt
					+ "</a>&nbsp;&nbsp;";
		}
		$(obj).append(link);
	...
	if (window.clipboardData) {
		...
		try {
			...
			$.fz_common
					.alert(
							"提示",
							"被浏览器拒绝！请在浏览器地址栏输入'about:config'并回车，然后将'signed.applets.codebase_principal_support'设置为'true'。");
		}
...
function setIPCTitle2(obj) {
	...
	if (load == null || load == false) {
		...
		$.ajax({
			...
			data : "select-key:simple_symbol="
					+ encodeURIComponent(span.text()),
			success : function(data) {

				ele.attr("load", "true");

				var en_title_text = $.fz_common.getXmlNodeValues(data,
						"context>record>en_title_text");
				var zh_title_text = $.fz_common.getXmlNodeValues(data,
						"context>record>zh_title_text");
	...
	if (load == null || load == false) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				var en_title_text = $.fz_common.getXmlNodeValues(data,
						"context>record>en_title");
				var zh_title_text = $.fz_common.getXmlNodeValues(data,
						"context>record>cn_title");
...
/** 记录日志* */
function saveSearchLog(search_type, search_library, express, builder,
		search_info, remark, options, action, fun, similary, express1) {
	if (action == "") {
		return;
	}
	...
	var _data = "log:search_type=" + search_type + "&log:search_library="
			+ encodeURIComponent(search_library) + "&log:express="
			+ encodeURIComponent(express) + "&log:builder="
			+ encodeURIComponent(builder) + "&log:search_info="
			+ encodeURIComponent(search_info) + "&log:remark="
			+ encodeURIComponent(remark) + "&log:options=" + options
			+ "&log:action=" + action + "&log:similary=" + similary
			+ "&log:express1=" + encodeURIComponent(express1);
	...
	$.ajax({
		...
		success : function(data) {
			...
			var errCode = $.fz_common.getXmlNodeValue(data,
					"context>error-code");
			if (errCode == "000000") {
				if (fun != null) {
					fun();
				...
				var errorDesc = $.fz_common.getXmlNodeValue(data,
						"context>error-desc");
	...
	(function() {
		...
		var _getStyle = function(el, prop) {
			...
			if (window.getComputedStyle) {
				value = window.getComputedStyle(el, null)
		...
		var _getDOMObjectPosition = function(obj) {
			...
			if (obj.getBoundingClientRect) {
				...
				if ("pageXOffset" in window && "pageYOffset" in window) {
					...
					pageXOffset = Math
							.round(document.documentElement.scrollLeft
									/ zoomFactor);
				...
				info.width = "width" in rect ? rect.width : rect.right
						- rect.left;
		...
		var _vars = function(options) {
			...
			if (origins.length) {
				str.push("trustedOrigins="
						+ encodeURIComponent(origins.join(",")));
			}
			if (typeof options.amdModuleId === "string" && options.amdModuleId) {
				str.push("amdModuleId="
						+ encodeURIComponent(options.amdModuleId));
			}
			if (typeof options.cjsModuleId === "string" && options.cjsModuleId) {
				str.push("cjsModuleId="
						+ encodeURIComponent(options.cjsModuleId));
			}
			return str.join("&");
		};
		...
		ZeroClipboard.prototype.ready = function() {
			try {
				var ready = this.htmlBridge
						.getAttribute("data-clipboard-ready");
		...
		ZeroClipboard.prototype.receiveEvent = function(eventName, args) {
			...
			case "load":
				if (args
						&& parseFloat(args.flashVersion.replace(",", ".")
								.replace(/[^0-9\.]/gi, "")) < 10) {
...
function norecordMessageChe() {
	if (typeof (mGrid_patentGrid_obj) == "undefined") {
		$("#norecordMessage")
				.html(
						"<h2 style=' font-weight:normal; font-size:28px;'><img src='/"
								+ rootPath
								+ "module/di/img/patent/peruseView/cheStrimg.png' /> 抱歉，根据您输入的检索词没能搜索到相关结果。</h2>");
		return false;
	}
...
function securityMessageChe(recordData) {
	...
	var securityCode = $.fz_common.getXmlNodeValue(recordData,
			"context>security");
	...
	if (errCode == "000000") {
		...
		var errorDesc = $.fz_common.getXmlNodeValue(recordData,
				"context>error-desc");
		return ("发生错误：" + errCode + "<p>" + errorDesc + "</p>");
	}

	return "<h2 style=' font-weight:normal; font-size:28px;'><img src='/"
			+ rootPath
			+ "module/di/img/patent/peruseView/cheStrimg.png' /> 抱歉，根据您输入的检索词没能搜索到相关结果。</h2>";
...
function securityListFun() {
	...
	for (var i = 0; i < accountList.length; i++) {
		...
		$(className).unbind("click");
		$(className).attr("href",
				'javascript:$.fz_common.alert("提示", "您的权限不足，请及时付费以获得更多权限。");');
	...
	if(typeof (mGrid_patentGrid_obj) != "undefined"){
		...
		var securityCode = $.fz_common.getXmlNodeValue(mGrid_patentGrid_obj.recordData,
		"context>security");
		
		if (securityCode != null && securityCode == "01") {
			var securityCode = $.fz_common.getXmlNodeValue(
					mGrid_patentGrid_obj.recordData, "context>security");
			
			var currentPage = $.fz_common.getXmlNodeValue(
					mGrid_patentGrid_obj.recordData, "context>currentPage");
			
			var startRow = $.fz_common.getXmlNodeValue(
					mGrid_patentGrid_obj.recordData, "context>startRow");
			
			var pageRow = mGrid_patentGrid_obj.opt.pageRow;
			var totalPages = parseInt(_record_number) / parseInt(pageRow) + 1;
			if (securityCode != null && securityCode == "01") {
				
				var opt = {
						currentPage : parseInt(currentPage),
						totalPages : parseInt(totalPages),
						totalNumber : _record_number,
						pageRow : pageRow,
						startRow : startRow
				}
				mGrid_patentGrid_obj.resetPaginator(opt);
				mGrid_patentGrid_obj.getScope().updatePaginator(opt);
			}
...
function _doTempPost(action, parameter, b, m) {
	...
	$("body").append(
			'<form action="' + action + '" style="display:none;" ' + _blank
					+ ' method="' + _method
					+ '" name="myPatentFormTmp" id="myPatentFormTmp"></form>');
	...
	$(ele).typeahead(
			{
				...
				source : function(query, process) {
					...
					$.ajax({
						...
						success : function(data) {
							var record = $.fz_common.getXmlNodeValues(data,
									"context>record");
							var arr = [];
...
var di_warning = function() {
	...
	function doTraWarning(tmid, rn, fd, mno, nice_class) {
		...
		$.ajax({
			...
			data : "select-key:tmid=" + tmid + "&select-key:rn=" + rn
					+ "&select-key:fd=" + (fd) + "&select-key:mno="
					+ encodeURIComponent(mno) + "&select-key:nice_class="
					+ (nice_class),
			success : function(data) {
	...
	function doWarning(pno, pid, ano, tio, watcher_status, ad) {
		...
		$.ajax({
			...
			data : "select-key:pno=" + pno + "&select-key:pid=" + pid
					+ "&select-key:ano=" + encodeURIComponent(ano)
					+ "&select-key:tio=" + encodeURIComponent(tio)
					+ "&select-key:watcher_status=" + watcher_status
					+ "&select-key:ad=" + encodeURIComponent(ad)
					+ "&select-key:sign_status="
					+ encodeURIComponent(watcher_status),
			success : function(data) {
	...
	$(".docWarningSubmit")
			.click(
					function() {
						...
						var warning_watcher_name = $("#warning_watcher_name")
								.val();
						if (warning_watcher_name == null
								|| warning_watcher_name == "") {
							$.fz_common.alert("提示", "请填写预警名称");
							return;
						}

						var watcher_type = $(
								'input:radio[name=record_watcher_type]:checked')
								.val();
						...
						di_warning_obj.warning(watcher_type, "1",
								warning_watcher_name);
					});
	...
	$(".addWarning").click(
			function() {
				...
				var txt = $.trim($("#select-key_expressCN").val()) + " "
						+ $.trim($("#select-key_expressCN2").val()) + " "
						+ getCategorySelectExpress();
				var express = $.trim($("#select-key_express").val()) + " "
						+ $.trim($("#select-key_expressCN2").val()) + " "
						+ getCategorySelectExpress();
	...
	return {
		...
		warning : function(watcher_type, watcher_status, watcher_name) {
			...
			$.ajax({
				...
				data : "record:watcher_type=" + watcher_type
						+ "&record:watcher_status=" + watcher_status
						+ "&record:watcher_name="
						+ encodeURIComponent(watcher_name)
						+ "&record:watcher_exp=" + encodeURIComponent(txt),
				success : function(data) {
...
var di_project = function() {
	...
	$(".addProjectAll")
			.click(
					function() {
						...
						if (searNum > 10000) {
							$.fz_common.alert("提示", "单次加入项目上限为10000条，当前检索结果数量为"
									+ searNum + "条，请重新检索缩小结果集后再新建项目。");
							return false;
						}
						$
								.ajax({
									...
									dataType : "xml",
									url : "/" + rootPath
											+ "txnCheckProjectNum.ajax",
									data : "select-key:project_num=" + searNum,
									success : function(data) {
										var errCode = $.fz_common
												.getXmlNodeValue(data,
														"context>error-code");
										if (errCode == "000000") {
											var sign = $.fz_common
													.getXmlNodeValue(data,
															"context>result>sign");
											// 单个项目的文献上限数
											var inProjectMaxNumber = $.fz_common
													.getXmlNodeValue(data,
															"context>result>inProjectMaxNumber");
											// 当前用户已经使用的文献数量
											var used_project_count = $.fz_common
													.getXmlNodeValue(data,
															"context>result>used_project_count");
											// 当前用户购买的文献总数
											var use_project_records = $.fz_common
													.getXmlNodeValue(data,
															"context>result>use_project_records");
			...
			function() {
				if (selectedNode != null) {
					if (!selectedProject["isParent"]) {
						_doTempPost("/" + rootPath + "txnGotoProject.do",
								"select-key:project_id="
										+ selectedProject["project_id"], true);
					}
				}
			});
	...
	$(".appendProjectConfirm").click(
			function() {
				...
				if (selectedNode != null) {
					...
					var _otherFunction = [ _addProjectFunctionOverride,
							_addProjectFunctionAppend ];

					$.fz_common.confirm("提示",
							"追加新数据到项目可能会有重复数据，对重复数据请选择您要进行的操作。", null,
							_otherButtons, _otherButtonStyles, _otherFunction);
			...
			.click(
					function() {
						if (selectedNode != null) {
							...
							var record_project_type = $("#record_project_type")
									.val();
							var record_project_name = $("#record_project_name")
									.val();
							...
							var _addProjectFunction = function() {
								...
								var project_id = (di_project_obj
										._selectedProject()["project_id"]);
								var projectTree = (di_project_obj
										._projectTree());
								var selectedNode = (di_project_obj
										._selectedNode());
								var selectedProject = (di_project_obj
										._selectedProject());
								...
								if (hisExp != "") {
									exp = hisExp;
									if (exp.substring(0, 4) == " AND") {
										exp = exp.substring(4, exp.length);
									}
								} else {
									exp = $.trim($("#select-key_expressCN")
											.val())
											+ " "
											+ $
													.trim($(
															"#select-key_expressCN2")
															.val())
											+ " "
											+ getCategorySelectExpress();
								}

								if (freezeTxnAction == "PatentNumberSea") {
									...
									$
											.ajax({
												...
												success : function(data) {
													$("#numberHitWindow")
															.hideLoading();
													var patent = $.fz_common
															.getXmlNodeValues(
																	data,
																	"context>record>hitnumber");
													number = $(patent).text()
															.replace(/\n\n/g,
																	"\n");
													numType = $.fz_common
													...
													$
															.ajax({
																...
																url : "/"
																		+ rootPath
																		+ "txnAddUserProjectByNumber.ajax",
																data : "select-key:parent_id="
																		+ project_id
																		+ "&select-key:name="
																		+ encodeURIComponent($(
																				"#record_project_name")
																				.val())
																		+ "&select-key:project_type="
																		+ encodeURIComponent($(
																				"#record_project_type")
																				.val())
																		+ "&select-key:number="
																...
																success : function(
																	...
																	if (errCode == "000000") {
																		...
																		$(
																				".AddProjectWindow .curSelectedNode")
																				.each(
																						function() {
																							$(
																									this)
												...
												error : function(data) {
													$.fz_common.alert("发生错误",
															data + "", null);
												}
											});

								} else if (exp.indexOf("fspns#") != -1) {
									$
											.ajax({
												type : "POST",
												dataType : "xml",
												url : "/"
														+ rootPath
														+ "txnPatentSFPNSList.ajax",
												data : "select-key:uuid="
														+ exp.replace("fspns#",
																"").replace(
																"(", "")
																.replace(")",
												...
												success : function(data) {
													$("#numberHitWindow")
															.hideLoading();
													var number = $.fz_common
															.getXmlNodeValues(
																	data,
																	"context>record>number")
															.text();
													var numType = $.fz_common
															.getXmlNodeValues(
																	data,
																	"context>record>numType")
															.text();
							...
							$.fz_common.confirm("提示", _confirmStr + "？", null,
									_otherButtons, _otherButtonStyles,
									_otherFunction);
										...
										.each(
												function(index, dom) {
													...
													data["parent_id"] = ele
															.find("parent_id")
															.text();
													data["project_name"] = ele
															.find(
																	"project_name")
															.text();
													data["folder_flag"] = ele
															.find("folder_flag")
															.text();

													if (ele.find("folder_flag")
															.text() == "0") {
														data["isParent"] = true;
													} else {
														data["isParent"] = false;
													...
													data["create_time"] = ele
															.find("create_time")
															.text();
													data["project_type"] = ele
															.find(
																	"project_type")
															.text();
	...
	var initTreeData = function(data, div_id) {
				...
				url : function(treeId, treeNode) {
					if (treeNode["project_id"] != "") {
						if (treeNode["project_id"] == "0") {
							return "/"
									+ rootPath
									+ "txnUserProjectTree.do?select-key:creatroot=false&select-key:parent_id="
									+ treeNode["project_id"]
						}
						return "/" + rootPath
								+ "txnUserProjectTree.do?select-key:parent_id="
								+ treeNode["project_id"]
					} else {
						// 不需要直接跟后台相应 数据如何写
		...
		if (div_id == null) {
			...
			projectTree = $.fn.zTree.init($("#addProjectTreeImg"), setting,
					data);
		...
		if (div_id == ".AddProjectWindow2") {
			...
			projectTree = $.fn.zTree.init($("#addProjectTreeImg2"), setting,
					data);
	...
	$(".addProjectNew").click(
			function() {
				...
				$(".addAppendTitle").removeClass("bgGray245").addClass(
						"bgAddProjectAppend");
				$(".addProjectNew").removeClass("bgAddProjectAppend").addClass(
						"bgAddProjectNew");
	...
	$(".addProjectAppend").click(
			function() {
				...
				$(".addProjectNew").removeClass("bgAddProjectNew").addClass(
						"bgAddProjectAppend");
				$(".addAppendTitle").addClass("bgGray245").removeClass(
						"bgAddProjectAppend");
			...
			.click(
					function() {
						...
						var _addProjectFunction = function() {
							...
							if (keySet.length == 0
									&& di_project_obj._oneDocAddProjectDocID() == "") {
								alert("没有选择内容");
								return;
							} else {
								...
								if (di_project_obj._oneDocAddProjectDocID() != "") {
									...
									list2 = di_project_obj
											._oneDocAddProjectDocID();
									listPno = di_project_obj
											._oneDocAddProjectTrsID();
								...
								$
										.ajax({
											...
											url : "/"
													+ rootPath
													+ "txnDocAddUserProject.ajax",
											data : "&select-key:project_id="
													+ (di_project_obj
															._selectedProject()["project_id"])
													+ "&record:pid="
													+ encodeURIComponent(list2)
													+ "&select-key:pno="
													+ encodeURIComponent(listPno)
													+ "&select-key:action_flag="
													+ action_flag,
											success : function(data) {
						...
						$.fz_common.confirm("提示", "确认加入项目？", null,
								_otherButtons, _otherButtonStyles,
								_otherFunction);
	...
	return {
		...
		addProjectTip : function(obj) {
			if($(obj).is(".missionListHitUnEnable")){
				return;
			}
			$("#numberJobTabClose").click();
			...
			if (hisNum > 10000) {
				$.fz_common.alert("提示", "单次加入项目文献数量上限为10000条，当前检索结果数量为"
						+ hisNum + "条，请重新检索缩小结果集后再新建项目。");
				return false;
			}
			...
			$.ajax({
				...
				success : function(data) {
					var errCode = $.fz_common.getXmlNodeValue(data,
							"context>error-code");
					if (errCode == "000000") {
						var sign = $.fz_common.getXmlNodeValue(data,
								"context>result>sign");
						// 单个项目的文献上限数
						var inProjectMaxNumber = $.fz_common.getXmlNodeValue(
								data, "context>result>inProjectMaxNumber");
						// 当前用户已经使用的文献数量
						var used_project_count = $.fz_common.getXmlNodeValue(
								data, "context>result>used_project_count");
						// 当前用户购买的文献总数
						var use_project_records = $.fz_common.getXmlNodeValue(
								data, "context>result>use_project_records");

						if (sign == "2") {
							$.fz_common.alert("提示", "单次加入项目文献数量上限为"
									+ inProjectMaxNumber + "条，当前检索结果数量为"
									+ hisNum + "条，请重新检索缩小结果集后再新建项目。");
							return false;
						} else if (sign == "1") {
							$.fz_common.alert("提示", "您当前可用"
									+ use_project_records + "条文献，该项目需添加"
									+ hisNum + "条文献,所以您的剩余文献条数不足");
							return false;
						} else if (sign == "3") {
						...
						$("#hitnumber").attr("uuid", $(obj).attr("uuid") + "");
					} else {
						errorDescAlert(data);
					}
				}
		...
		_addProjectFunction : function(action_flag) {
			...
			if (hisExp != "") {
				...
				exp = $.trim($("#select-key_expressCN").val()) + " "
						+ $.trim($("#select-key_expressCN2").val()) + " "
						+ getCategorySelectExpress();
			}
			...
			$
					.ajax({
						...
						data : "select-key:project_num="
								+ $("#contentTipNumList").text()
								+ "&select-key:project_id=" + project_id,
						success : function(data) {
							var errCode = $.fz_common.getXmlNodeValue(data,
									"context>error-code");
							if (errCode == "000000") {
								...
								var use_project_records = $.fz_common
										.getXmlNodeValue(data,
												"context>result>use_project_records");
								if (sign == "2") {
									$.fz_common.alert("提示", "每个项目的最大文献记录数为"
											+ inProjectMaxNumber
											+ "条，当前检索结果数量为"
											+ $("#contentTipNumList").text()
											+ "条，" + "待追加项目已有"
											+ used_project_count
											+ "条文献，请重新检索缩小结果集后再新建项目。");
													...
													success : function(data) {
														...
														$
																.ajax({
																	...
																	url : "/"
																			+ rootPath
																			+ "txnAppendUserProjectByNumber.ajax",
																	timeout : 1000 * 60 * 3,
																	data : "select-key:action_flag="
																			+ action_flag
																			+ "&select-key:parent_id="
																			+ project_id
																			+ "&select-key:project_id="
																			+ project_id
																			+ "&select-key:project_type=01&select-key:hitNumber="
																			+ number
																			+ "&select-key:type="
																			+ numType,
																	success : function(
										...
										$
												.ajax({
													...
													data : "select-key:action_flag="
															...
															+ "&select-key:project_id="
															+ project_id
															+ "&select-key:project_type=01&select-key:exp="
															+ encodeURIComponent(exp)
															+ "&select-key:project_num="
															+ $(
																	"#contentTipNumList")
																	.text(),
													success : function(data) {
...
function fieldExpress(input, button, sign, m_str) {
	...
	if (t.indexOf("+") > 0 || t.indexOf(",") > 0) {
		...
		if ($.trim(v).indexOf("\\=") == 0 || $.trim(v).indexOf("\\<") == 0
				|| $.trim(v).indexOf("\\>") == 0
				|| $.trim(v).indexOf("\\!") == 0) {
...
function getExpressValue(v, t, sign){
...
function changeBlank(v){
	var vs = v.split(" ");
	var result = vs[0];
	var operator = ["AND", "OR", "NOT"];
	
	for(var index = 1; index < vs.length; index ++){
		if($.inArray(vs[index -1].toUpperCase(), operator) == -1 && $.inArray(vs[index].toUpperCase(), operator) == -1){
			result = result + " AND " + vs[index];
		}else{
			result = result + " " + vs[index].toUpperCase();
		}
	}
	return result;
}
...
function _expressExpMes(data) {
	...
	var _express = $.fz_common.getXmlNodeValue(data,
			"context>select-key>express");
	var _expressExpMes = $.fz_common.getXmlNodeValue(data,
			"context>select-key>expressExpMes");
	var _expressExpPos = $.fz_common.getXmlNodeValue(data,
			"context>select-key>expressExpPos");
	...
	try {
		...
		_expressSub = " ( <b style=\"color:#FF0000\">"
				+ _express.substr(_expressExpPosStart, _expressExpPos)
				+ "</b> ) ";
	...
	$.fz_common.alert("提示", "<p>当前表达式存在错误：</p>" + "<p><b>" + _expressExpMes
			+ "</b></p>" + "<p>位置：" + _expressExpPos + _expressSub + "</p>",
			_dialogHidden);
...
//检索条件处理，将非引号括号里面的单空格用“AND”替换掉
function changeBlankToAND(value){
...
//显示
	...
	$(".shielding_layer").removeClass("displayNone");
	$(".bodyClass").addClass("overflowHidden");
	$("#onlinecashwindow").removeClass("displayNone");
}
//关闭
$("#onlinecashclose").click(function(){
	MouseWheel();
	clearInvoice();
	clearMoney();
	$(".shielding_layer").addClass("displayNone");
	$(".bodyClass").removeClass("overflowHidden")
	$("#onlinecashwindow").addClass("displayNone");
...
//金额
$("#date_number").keyup(function(){
	
	var date_number = $("#date_number").val();
	var reg = /^\+?[1-9][0-9]*$/;
	if(!reg.test(date_number)){
		alert("请输入有效数字。");
		return;
	}
	if(date_number > 365 ){
	...
	var library = $("input[name='library']:checked").length;
	var param ="input:date_number="+date_number+"&input:library="+library;
	$.ajax({
		type : "POST",
		dataType : "xml",
		url : "/" + rootPath + "txnGetMoney.ajax",
		data : encodeURI(param).replace(/\+/g, "%2B"),
		success : function(data) {
			var errCode = $.fz_common.getXmlNodeValue(data, "context>error-code");
			if (errCode == "000000") {
				var money = $.fz_common.getXmlNodeValue(data, "context>record>money");
				...
				//var errorDesc = $.fz_common.getXmlNodeValue(data, "context>error-desc");
				//$.fz_common.alert("错误", errCode + "<p>" + errorDesc + "</p>", null);
			}
		},
		error : function(data) {
			//$.fz_common.alert("错误", data + "", null);
...
$("#ali").click(function(){
	...
	var open_url ="/freeze.main?txn-code=ssoUserLogin&redirect=txn801232.do&req=select-key:order_type=vip&req="; 
	getOrderId(open_url);
...
$("#wx").click(function(){
	...
	var open_url ="/freeze.main?txn-code=ssoUserLogin&redirect=txn801244.do&req=select-key:order_type=vip&req="; 
	getOrderId(open_url);
...
//支付宝提交
function getOrderId(open_url){

	var date_number = $("#date_number").val();
	var r = /^\+?[1-9][0-9]*$/;
	if(!r.test(date_number)){
		alert("请输入有效数字");
		return;
	}
	if(date_number >365 ){
	...
	var param ="input:date_number="+date_number+"&input:library="+library +"&input:money="+money;
	$.ajax({
		type : "POST",
		dataType : "xml",
		url : "/" + rootPath + "txnCreateOrder.ajax",
		data : encodeURI(param).replace(/\+/g, "%2B"),
		success : function(data) {
			var errCode = $.fz_common.getXmlNodeValue(data, "context>error-code");
			if (errCode == "000000") {
				var order_id = $.fz_common.getXmlNodeValue(data, "context>record>order_id");
				
					//返回订单信息 调用 用户支付接口
					if(order_id){

						if($("#invCkb").is(':checked')){
							//发票
							commitInvoice(open_url,order_id);
						}else{
							//支付
							var param = "select-key:order_id="+order_id;
							pay_url(open_url+param);
						}
						
					
					}else{
						$.fz_common.alert("错误", "生产数据信息错误！");
						return;
					}
...
//发票
function commitInvoice(open_url,order_id){
	
	var phone = $("#phone").val();
	var linkman = $("#linkman").val();
	var companyName = $("#companyName").val();
	var mailing_address = $("#mailAdd").val();
   
   if("" == companyName){
	   alert("单位名称不能为空，请输入单位名称。");
	   return;
   }
   if("" == linkman){
   ...
   if("" == phone){
	...
	var param ="record:detailed_address="+mailing_address+"&record:u_order_id="+
	   order_id +"&record:linkman="+linkman+"&record:phone_number="+phone+"&invoice_title="+companyName;
	$.ajax({
		type : "POST",
		dataType : "xml",
		url : "/" + rootPath + "txnAddInvoice.ajax",
		data : encodeURI(param).replace(/\+/g, "%2B"),
		success : function(data) {
			var errCode = $.fz_common.getXmlNodeValue(data, "context>error-code");
			if (errCode == "000000") {
   ...
   $("#onlinecashclose").click();//关闭弹框
}

$("#invCkb").change(function() {
    if($(this).is(':checked')){
        $("#companyName").attr("disabled",false);
        $("#linkman").attr("disabled",false);
        $("#mailAdd").attr("disabled",false);
