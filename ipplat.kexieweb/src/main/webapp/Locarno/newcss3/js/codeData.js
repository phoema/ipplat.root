		...
		$(".btnWordSearch").click(function (){
			...
			if($(".Crosslanguage").hasClass("typeSelect")){
				...
				if($(".personWord").hasClass("typeSelect")){
					var param = getParam(lang,value,"1");
				...
				if($(".systemWord").hasClass("typeSelect")){
					var param = getParam(lang,value,"2");
					getWord("txnGetCrossLanguage",param,"lang");//跨语言
		...
		function getParam(lang,value,type){
			return	param = "select-key:chinese_word="+ value +"&select-key:vocabulary_type="+type+"&select-key:lang="+lang;
...
//			treeDiv.addClass("displayNone")
		...
		$(".areaSearch").click(function() {
			var windowForm = "applicantWindow";// 当前窗口
			var searchText = $(this).next().val();// 查询内容
			if(""==$.trim(searchText)){
				initZTree($("#applicantTreeDemo"), regsetting, zPetitionerNodes, "petitionerjson", "");
				//删除提示
				$("#_applicantNull").addClass("displayNone");
				return;
			}
			
			var treeDiv = $("#applicantTreeDemo");// 国内
			var btnType = $(this).attr("v");// 国内或国外
			...
			if("tools" == winType){
				...
				applicant_Setting.check.enable = false;//工具栏
			}
			
			$(treeDiv).children().remove();//删除原有树//检索效果为树时sh
			
			try{
				queryCompanyData("txnApplicantTree.ajax","select-key:text=" + searchText +"&select-key:level=-1");//系统查询
			...
			if(nodes){
				$("#_applicantNull").addClass("displayNone");
				$(treeDiv).removeClass("displayNone");
				 areacode_node = eval('(' + nodes + ')');
				$.fn.zTree.init(treeDiv, applicant_Setting, areacode_node);
				//将检索到的内容展开
				var treeObj = $.fn.zTree.getZTreeObj("applicantTreeDemo");
				var nodes = treeObj.getNodes();
				if (nodes.length>0) {
				...
				$("#_applicantNull").removeClass("displayNone");
				$("#applicantTreeDemo").children().remove();
				$("#applicantTreeDemo").addClass("displayNone");
			}
		});
		
		//申请人区域代码导航
		function loadAreaMenu(param){
			$.ajax({
				type : "POST",
				dataType : "xml",
				async:false,
				url : "/" + rootPath +   "txnPetitionerState.ajax?",
				data :   encodeURI(param),
				success : function(data) {
					$("#applicantWindow").hideLoading();
		...
		function createAreaMenu(arr){
			$(arr).each(
				...
				function(index, dom) {
					...
					if ( ele.find("state").text() != "") {
						//构建导航
						existsLi(ele.find("classid").text(),ele.find("state").text());
					}
		...
		function selectAreaList(state){
			$("#areaName").val(state);
			mGrid_gridArea_obj.query(1, function() {// 异步加载
				//$("#applicantWindow .modelGridPaginatorContainer li span").attr("href", "javascript:void(0);");// 锚点
		 ...
		 * @param classid
		 */
		var style2 =" style='cursor: pointer;'";
		function addstate(classid,state){
			var style =" style='margin-left: 30px;line-height:20px;font-size:14px;color: rgb(59, 149, 242);'";
			var li ="";
			if(classid == "A"){
				li +="<div "+style+" class='a1'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onclick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "B"){
				li +="<div "+style+" class='a2'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "F"){
				li +="<div "+style+" class='a3'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "G"){
				li +="<div "+style+" class='a4'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "H"){
				li +="<div "+style+" class='a5'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "J"){
				li +="<div "+style+" class='a6'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "L"){
				li +="<div "+style+" class='a7'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "N"){
				li +="<div "+style+" class='a8'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "Q"){
				li +="<div "+style+" class='a9'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "S"){
				li +="<div "+style+" class='a10'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "T"){
				li +="<div "+style+" class='a11'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "X"){
				li +="<div "+style+" class='a12'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "Y"){
				li +="<div "+style+" class='a13'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			if(classid == "Z"){
				li +="<div "+style+" class='a14'>"+classid+"&nbsp;&nbsp;&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span></div>";
			}
			$("#applicantWindow .typePageItem").append(li);
		}
		 
		function existsLi(classid,state){
			if(classid == "A" && $("div.a1").length > 0){
				$("div.a1").append("&nbsp;<span  onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			} else if(classid == "B" && $("div.a2").length > 0){
				$("div.a2").append("&nbsp;<span  onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			}else  if(classid == "F" && $("div.a3").length > 0){
				$("div.a3").append("&nbsp;<span "+style2+"  onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			}else if(classid == "G" && $("div.a4").length > 0){
				$("div.a4").append("&nbsp;<span "+style2+"  onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			}else if(classid == "H" && $("div.a5").length > 0){
				$("div.a5").append("&nbsp;<span "+style2+"  onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			}else if(classid == "J" && $("div.a6").length > 0){
				$("div.a6").append("&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			}else if(classid == "L" && $("div.a7").length > 0){
				$("div.a7").append("&nbsp;<span "+style2+"  onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			}else if(classid == "N" && $("div.a8").length > 0){
				$("div.a8").append("&nbsp;<span "+style2+"  onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			}else if(classid == "Q" && $("div.a9").length > 0){
				$("div.a9").append("&nbsp;<span "+style2+"  onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			}else if(classid == "S" && $("div.a10").length > 0){
				$("div.a10").append("&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			}else if(classid == "T" && $("div.a11").length > 0){
				$("div.a11").append("&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			}else if(classid == "X" && $("div.a12").length > 0){
				$("div.a12").append("&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			}else if(classid == "Y" && $("div.a13").length > 0){
				$("div.a13").append("&nbsp;<span "+style2+" onClick='selectAreaList(\""+state+"\");'>"+state+"</span>");
			}else if(classid == "Z" && $("div.a14").length > 0){
				$("div.a14").append("&nbsp;<span "+style2+" onClick='selectAreaList('\""+state+"\"');'>"+state+"</span>");
			}else{
				addstate(classid,state);
			}
		...
		$("button.btnlawstatus").click(function() {
			var windowId = $(this).attr("v");
			var value = getAreaItemValueQuotes($("#"+windowId+" .Conditions1").children("div"));//获取选中的内容
		...
		//申请人区域代码加入检索
		$("#btnPetitionerArea").click(function() {
			
			var windowId = $(this).attr("v");
			var value = addCNToValue($("#"+windowId+" .Conditions1").children("div"));//添加CN和%
			addConditionsArea(value, windowId);// 加入输入框
			$("input.searchInputText").val(""); // 清空查询内容
				...
				$(crossLanguage).each(
						function(index, dom) {
							...
							if("1" == lang ){
								...
								"</td>" +
								"<td class=' '> " ;
								if(dom.english_word.indexOf(",") != -1){//多条
		...
		$(".addToWord2").click(function(){
			...
			for ( var i= 0;  i< tr.length; i++) {
				...
				var  word1 = $(array_element).find(".num1 .checkBoxClickBg").next("span").text();
				
				var  wordList = $(array_element).find(".num2 .checkBoxClickBg").next("span");
				var word2 = getWordList(wordList);
				if( $(array_element).find("span").hasClass("checkBoxClickBg")){//判断是否有选中内容 checkBoxStyle
				
					if(null == cnList || cnList.length == 0){
						
						$(wordItem).append("<div  class='wordCnList' style='width:150px;float:left;height: 20px; line-height: 20px; margin-left: 10px; margin-right: 5px;'>" + word1 +"</div>");//中
						$(wordItem).append("<div  class='wordEnList' style='width:160px;float:left;height: 20px; line-height: 20px; margin-left: 10px; margin-right: 5px;' v='"+word2+"'>" + word2.replace(/[$$$$]/g," ") +" </div>");//英
					}else{
						var s = false;
		...
		$(".confirmWordDiv2").click(function(){
			...
			try{
				for(var j = 0 ;j <cnList.length;j++){
					var param = "record:chinese_word="+ $(cnList[j]).text() +"&record:english_word="+ $(enList[j]).attr("v") +"&record:vocabulary_type=1";//跨语言
					getWord("txnInsertPersonCL",param,"lang");
					$.fz_common.alert("提示","添加成功。");
				}
		...
		$("#petitionerCancel").click(function() {
			closeAreaWindow();
		});
		
		function closeAreaWindow() {
			MouseWheel();
			$("#applicantWindow .searchInputText").val("");
			$("#applicantWindow").addClass("displayNone");
			$(".shielding_layer").addClass("displayNone");
			$(".bodyClass").removeClass("overflowHidden");
		}
...
//						var param="select-key:codename="+treeNode.name+"&select-key:level="+(treeNode.level);//+"&select-key:nodeName="+ stripscript($("#s_value").val())
		...
		var chinaRegionSetting = {
				...
				async: {
					...
					url : function(treeId, treeNode) {
						...
						var nodeName = "select-key:nodeName="+treeNode.name;
						if(treeNode.getParentNode()){
							nodeName = "select-key:nodeName="+treeNode.getParentNode().name+"/"+treeNode.name;
						}
						var param=nodeName+"&select-key:level="+(treeNode.level)+"&select-key:text="+ stripscript($("#regionText").val());
						return txnCode + param;
					},
					autoParam : [ "level", "name" ],
					dataFilter : null
					// 异步返回后经过Filter
				},
				callback : {
		...
		var applicant_Setting = {
				...
				async: {
					...
					type : "post",
					url : function(treeId, treeNode) {
						var txnCode= "/" + rootPath + "txnApplicantTree.do?";
						var nodeName = "select-key:nodeName="+ getNodeName(treeNode.name);
						if(treeNode.getParentNode()){
							nodeName = "select-key:nodeName="+getNodeName(treeNode.getParentNode().name)+"/"+getNodeName(treeNode.name);
						}
						var param=nodeName+"&select-key:level="+(treeNode.level)+"&select-key:text="+ stripscript($("#applicant_Area_Text").val());
						return txnCode + param;
					},
					autoParam : [ "level", "num" ],
					dataFilter : null
			...
			if($(list).length>0){
				...
				for(var i= 0 ;i<$(list).length;i++){
					...
					if(i==0){
						...
						value=getApplicantNum($(list).eq(i).attr("v"));
		...
		function getApplicantNum(value){
			if(value.length<6){
				return "CN" + value + "%";
			}else{
				return "CN" + value ;
		...
		$("#_theIpc,#_theCpc,#_theFi,#_theFterm,#_theUc").click(function(){
			var isChecked = $(this).prop("checked");
			var winForm = $(this).attr('v');
			$("input[name='ckbx']").prop("checked", isChecked);
			$("input[name='ckbx']").each(function() {
				
				addIpcArea(this,winForm);
			});
