		...
		.ready(
				function() {
					...
					// 添加链接
					{
						...
						if (txncode == "StandardDetail" || txncode == "StandardDoubleCompare" || txncode == "TradeMarkDetail" || txncode == "UsTradeMarkDetail" || txncode == "MdTradeMarkDetail"
							...
							if (!jourAuthor.indexOf(";")) {
								...
								for ( var i = 0; i < jourAuthors.length; i++) {
									text = encodeURIComponent(jourAuthor[i]);
									href = "txnPeriodicalList.do?select-key%3AexpressCN=(作者='" + text.replace(/\'/g, "\\'") + "')";
					...
					// 相关数据隐藏项
					{
						var similary = true;
						// 相关商标
						if ($(".similaryTra").length == 0) {
							$(".similaryTra1").addClass("displayNone");
							$(".similaryTra2").addClass("displayNone");
							$(".similaryTra3").addClass("displayNone");
						} else {
							similary = false;
						}
						...
						// 相关专利
						if ($(".similaryPat").length == 0) {
							$(".similaryPat1").addClass("displayNone");
							$(".similaryPat2").addClass("displayNone");
							$(".similaryPat3").addClass("displayNone");
						} else {
							similary = false;
						}
					...
					// 导航栏滚动 通用隐藏显示
					{
						...
						$(".classificationIcon").click(function() {
							if ($(this).parent().next().hasClass("displayNone")) {
								$(this).css("background", "url(/" + rootPath + "module/di/img/patent/peruseView/ic1.png)");

								$(this).css("background-repeat", "no-repeat");
							} else {
								$(this).css("background", "url(/" + rootPath + "module/di/img/patent/peruseView/icup.png)");
								$(this).css("background-repeat", "no-repeat");
							}
					...
					// 通用链接重置 格式化等
					{
						...
						$(".filterDay1").each(function() {
							var txt = $(this).text();
							if(txt.length == 8 || txt.length == 19 || txt.length == 10){
						...
						$(".filterDay2").each(function() {
							var txt = $(this).text();
							if(txt.length == 10 ){
								$(this).text(_dataFilterDayKong(txt));
							}
