...
//重置
function resetinfo(){
	var treeObj = $.fn.zTree.getZTreeObj("locanoTreeDemo");
...
function selectinfo(){
	...
	// 对应字段表达式 
	$("#expresswg").val(expresswg);
	var m_str  = getFieldExpress(m_str,'#expressdiv'); 
	
	//layer.alert('m_str***'+m_str); 
	
	var expressTxt = m_str;
	
	// 对应库
	var le_str = getLibsExpress();
	//layer.alert('le_str****'+le_str); 
 
var finalex=expressTxt+ " AND ("+le_str+")";
//layer.alert('preCheckExpress***'+finalex);

finalex=finalex.replace('undefined','')
   window.location.href="../list/patentlist.html?ex="+encodeURIComponent(finalex);
}
//预检索地址
