...
(function($) {
	...
	function addTreeDom(setting, treeNode) {
		if (setting.dragStatus == 0) {
			...
			if(setting.searchable){
				if(setting.mixsearch){
					if(treeNode.expMix)addMIXBtn(setting, treeNode);
				}else{
					if(treeNode.expCn)addCNBtn(setting, treeNode);
					if(treeNode.expEn)addENBtn(setting, treeNode);
				}
			}
			/********* add for cnipr end ******************/
	...
	function removeTreeDom(setting, treeNode) {
		...
		removeCNBtn(treeNode);// cnipr btn
		removeENBtn(treeNode);// cnipr btn
		removeMIXBtn(treeNode);// cnipr btn
	...
	function removeCNBtn(treeNode) {		
		$("#" + treeNode.tId + "CN").unbind().remove();
	}
	function removeENBtn(treeNode) {		
		$("#" + treeNode.tId + "EN").unbind().remove();
	}
	function removeMIXBtn(treeNode){
		$("#" + treeNode.tId + "_mix").unbind().remove();
	}
	
	// add for cnipr
	function addCNBtn(setting, treeNode){
		if (treeNode.editNameStatus || $("#" + treeNode.tId + "CN").length > 0) {
			return;
		}
		var nObj = $("#" + treeNode.tId + IDMark_Span);
		var editStr = "<button type='button' class='cn' id='" + treeNode.tId + "CN" + "' title='中文专利' treeNode"+IDMark_Edit+" onfocus='this.blur();' style='display:none;'></button>";
		nObj.before(editStr);
		
		$("#" + treeNode.tId + "CN").bind('click', 
			function() {
			  doExpSearch(treeNode.id,"CN");
			}
		).show();
	}
	// add for cnipr
	function addMIXBtn(setting, treeNode){
		if (treeNode.editNameStatus || $("#" + treeNode.tId + "_mix").length > 0) {
			return;
		}
		var nObj = $("#" + treeNode.tId + IDMark_Span);
		var editStr = "<button type='button' class='mix' id='" + treeNode.tId + "_mix" + "' title='' treeNode"+IDMark_Edit+" onfocus='this.blur();' style='display:none;'></button>";
		nObj.before(editStr);
		
		$("#" + treeNode.tId + "_mix").bind('click', 
				function() {
			doExpSearch(treeNode.id,"MIX");
		}
		).show();
	}
	// add for cnipr
	function addENBtn(setting, treeNode){
		if (treeNode.editNameStatus || $("#" + treeNode.tId + "EN").length > 0) {
			return;
		}
		var nObj = $("#" + treeNode.tId + IDMark_Span);
		var editStr = "<button type='button' class='en' id='" + treeNode.tId + "EN" + "' title='英文专利' treeNode"+IDMark_Edit+" onfocus='this.blur();' style='display:none;'></button>";
		nObj.before(editStr);
		
		$("#" + treeNode.tId + "EN").bind('click', 
			function() {
			  doExpSearch(treeNode.id,"EN");
			}
		).show();
	}
