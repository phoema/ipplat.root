...
$(document).ready(function() {
    ...
    $('#expressCN').val('');
		...
		// 自定义字段
		$(ele).each(function() { 
			var  count= $(this).find("input[type = 'text']").length;
	...
	$("#CheckMoreFiled").click(function() {
		...
		var left=$("#CheckMoreFiled").position().left; 
		var top=$("#CheckMoreFiled").position().top; 
	    
		$("#fieldListWindow").css("position","absolute"); 
		$("#fieldListWindow").css("left", (left+30)+ "px");
		$("#fieldListWindow").css("top", (top+38)+ "px");
...
function getLibsExpress() {
	...
	// 专利对应库
	var m_str = "";
	$("#ddul li").each(function(index, domEle) {
		//var v = $(this).attr("v");
		var v = $(this).html();
		if (v != null && v != "") {
			...
			if (v != "OTHE") {
				...
				//v = "PDB = 'OTHE'";
				//修正表达式
				v ="(PDB = 'OTHE' OR "+ "(PDB !='DD%' AND PDB !='US%' AND PDB !='JP%' AND PDB !='CN%' AND PDB !='AU%' AND PDB !='CA%' AND PDB !='CH%' AND PDB !='DE%' AND PDB !='EP%' AND PDB !='FR%' AND PDB !='GB%' AND PDB !='HK%' AND PDB !='IB%' AND PDB !='KR%' AND PDB !='TW%' AND PDB !='RU%' AND PDB !='WO%' AND PDB !='MO%') )";
			}
	...
	$(ele).each(function() {
		...
		if (!($(this).hasClass("displayNone"))) {
			...
			if (input != null && $(input).val() != null && $.trim($(input).val()) != "" && $.trim($(input).attr("title")) != "") {
				...
				m_str = fieldExpress(input, "AND", sign, m_str);
