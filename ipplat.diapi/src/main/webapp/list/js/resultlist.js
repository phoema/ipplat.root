					 ...
					 for(var i=0;i<objrecord.length;i++){ 
						 ...
						 /*处理法律状态start*/
				     	 ...
				     	 str+='<div class="secondDiv ng-scope" ng-repeat="patent in data">';
						   ...
						   str+=((currentpage-1)*10+(i+1))+'、<a  href="../detail/txnpatentDetail.html?pid='+objrecord[i].pid+'"  target="_blank" class="signFilter TIChighlight languageSelectField account_state_1_link _link ng-binding">';
						   str+=fixnull(objrecord[i].tio)+'</a></div>';          
						   str+='<div class="displayInline" style="float:right;">';           
						  
						 if(!!objrecord[i].lssc&&objrecord[i].lssc!=0&&objrecord[i].lssc!=4){ 
						 ...
						 if(objrecord[i].pdt!=''&&objrecord[i].pdt!=null&&objrecord[i].pdt!='null'){
						   ...
						   str+='<div style=" width:300px;" class="displayInline arial neirong ng-binding">'+objrecord[i].ans+'</div>';  
						   ...
						   if(objrecord[i].pdt=="外观设计"){
							   ...
							   str+='<img onerror="imgonerror(this);" onclick="tobig(this,\''+objrecord[i].IMGO+'\')" alt="缩略图" style="width:214px; height:194px;" title="缩略图" src="'+objrecord[i].IMGO+'" class="im1 smallImgDisplay"></div>';   
							   str+='</div></div>';
							   str+='</div>'; 
...
/*点击申请人 跳转到相关申请人的列表 不再要之前的限制了
 * pdb不用传  
 * 点击申请人：申请人=（宁波市文魁控股集团有限公司） 
 * 申请人=(菲尼克斯亚太电气(南京)有限公司) 异常，需加单引号
*/
function appclick(e){
	var appname=$.trim($(e).html()); 
	var url=location.href;
	var urlfront=decodeURIComponent(url.split('?')[0]); 
	var urlfinal=urlfront+"?ex=申请人=('"+appname+"')"; 
	window.open(urlfinal);  
}
/*点击洛迦诺 跳转到洛迦诺的列表 不再要之前的限制了
 * pdb不用传   
 * 洛加诺:ex=(洛迦诺=('19-06'))
*/
function Lclick(e){
	...
	var url=location.href;
	var urlfront=decodeURIComponent(url.split('?')[0]); 
	var urlfinal=urlfront+"?ex=洛迦诺=('"+appname+"')"; 
	window.open(urlfinal);   
}
/*点击IPC 跳转到IPC的列表 不再要之前的限制了
 * pdb不用传  
 * 点击IPC：IPC=（） 
*/
function IPCclick(e){
	...
	var url=location.href;
	var urlfront=decodeURIComponent(url.split('?')[0]); 
	var urlfinal=urlfront+"?ex=IPC=('"+appname+"')"; 
	window.open(urlfinal);   
}
