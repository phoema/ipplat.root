...
function getlist(currentpage,union){ 
   ...
   var str='';  
	$.ajax({ 
		type : "post",
		dataType : "json",  
        url : "../api/copyright/work/search/expression", 
		data:{
			 express: finalEx, 
			 page:currentpage,
					...
					var  objrecord=eval( obj.records ); 
					 if(objrecord!==undefined&&objrecord!='undefined'){
						 for(var i=0;i<objrecord.length;i++){ 
							  ...
							  str+='<table style=" width:92%;" border="0" cellpadding="0" cellspacing="0">';
							  str+='<tbody>';
							   
							  str+='<tr>';
							  str+='<td class="ng-binding" style=" width:50%;"><b>登记号：</b>'+ objrecord[i].RN+'</td>';
							  str+='<td class="ng-binding"><b>著作权人：</b>'+ fixnull(objrecord[i].OWNER)+'</td></tr>';
							  str+='<tr><td class="ng-binding"><b>作品名称：</b>'+ fixnull(objrecord[i].ANM)+'</td>';
							  str+='<td class="ng-binding"><b>作品类别：</b>'+ fixnull(objrecord[i].TYPE)+'</td></tr>';
							  str+='<tr>';
							  str+='<td class="ng-binding" style=" width:50%;"><b>创作完成日期：</b>'+ fixdate(objrecord[i].FD)+'</td>';
							  str+='<td class="ng-binding"><b>首次发表日期：</b>'+ fixdate(objrecord[i].FPD)+'</td></tr>';
							  str+='<tr><td class="ng-binding"><b>登记日期：</b>'+ fixdate(objrecord[i].RD)+'</td>';
							  str+='<td class="ng-binding"><b>发布日期：</b>'+ fixdate(objrecord[i].PD)+'</td></tr>';
...
function initleft(){     
	...
	var finalEx = express + leftEx;
	$.ajax({
		url:'../api/copyright/work/statistics',
		type:'post',
		dataType:'json',
		data:{
			express:finalEx
		...
		success:function(sender){
			...
			if(obj=='null'|| obj==null){
				...
				/*首次发表年*/
				bulidLeft($('#FPY'),obj.FPY);
				/*创作完成年*/
				bulidLeft($('#FY'),obj.FY);
				/*作品类别*/
				bulidLeft($('#TYPE'),obj.TYPE);
				/*著作权人*/
				bulidLeft($('#OWNER'),obj.OWNER);
