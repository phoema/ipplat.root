...
//jia 相关数据重置链接
function buildLinkRelation(obj, url, union_data,express) {
	
	//window.open("trademarklist.html?union_data="+union_data+"&ex=" + trade_express);  
	var index = express.indexOf("+=");
	var keyword = express.substring(index);
	var patent_express = "AN,AD,PN,PD,APO,INO,TIO,ABSO" + keyword;
	var trade_express = "RN,RD,SN,FD,MN,HN" + keyword;
	var soft_express = "RN,CTN,PDF,RD,SWP,SWFN,SWSN" + keyword;
	var work_express = "RN,RD,ANM,OWNER,AUTHOR,PD,FPD,FD" + keyword;
	var total_array = union_data.split(";");
	...
	if (txt != null && txt != ("")) {
		...
		var link = "";
		if ($.trim(txt) != "" && txt != "0") {
			link = "<a href=\"" + url + "?union=true&union_data="+encodeURIComponent(union_data)+"&ex=" + encodeURIComponent(express)
...
//jia 大数据关联显示条
function buildAllLinkRelation(union_data,express){
	...
	if (!!total_array[0] && $("#relationPATNUM")) {
		$("#relationPATNUM").text(total_array[0]);
		buildLinkRelation($("#relationPATNUM"),"patentlist.html",union_data,patent_express)
	}
	if (!!total_array[1] && $("#relationTRANUM")) {
		$("#relationTRANUM").text(total_array[1]);
		buildLinkRelation($("#relationTRANUM"),"trademarklist.html",union_data,trade_express)
		
	}
	if (!!total_array[2] && $("#relationSOFTNUM")) {
		$("#relationSOFTNUM").text(total_array[2]);
		buildLinkRelation($("#relationSOFTNUM"),"crsoftlist.html",union_data,soft_express)
	}
	if (!!total_array[3] && $("#relationWORKNUM")) {
		$("#relationWORKNUM").text(total_array[3]);
		buildLinkRelation($("#relationWORKNUM"),"crworklist.html",union_data,work_express)
	}
